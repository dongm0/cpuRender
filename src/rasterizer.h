...
namespace rst {
    struct SoftFragmentShaderPayload {
        ...
        SoftFragmentShaderPayload(const Eigen::Vector3f &v, const Eigen::Vector3f &c, const Eigen::Vector3f &n, const Eigen::Vector2f &tc) : 
    ...
    public:
        ...
        void SetFragmentShader(std::function<Eigen::Vector3f(SoftFragmentShaderPayload)> fs) { fragmentShader = fs; }
        void SetVertexShader(std::function<Eigen::Vector3f(SoftVertexShaderPayload)> vs) { vertexShader = vs; }
    ...
    private:
        void rasterizeTriangle(const Triangle& t, std::array<Eigen::Vector4f, 3> view_pos);
        void rasterizeLine(Eigen::Vector4f& st, Eigen::Vector4f& ed);
    };

    Eigen::Matrix4f GetModelMatrix(Eigen::Vector3f base, Eigen::Quaternionf rotation, float scale) {

    }
    Eigen::Matrix4f GetModelMatrix(Eigen::Vector3f base, float scale) {
        Eigen::Matrix4f res;
        res << scale, 0, 0, -base[0],
            0, scale, 0, -base[1],
            0, 0, scale, -base[2],
            0, 0, 0, 1;
        return res;
    }
    Eigen::Matrix4f GetViewMatrix(Eigen::Vector3f base, Eigen::Vector3f lookat, Eigen::Vector3f head) {
    }
    Eigen::Matrix4f GetViewMatrix(Eigen::Vector3f base) {
        Eigen::Matrix4f res;
        res << 1, 0, 0, -base[0],
            0, 1, 0, -base[1],
            0, 0, 1, -base[2],
    ...
    Eigen::Matrix4f GetProjMatrix(float fovy, float asp_ratio, float zn, float zf) {
